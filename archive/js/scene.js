...
scene.background = new THREE.Color(0, 0, 0); //prev: 'white'
...
if (globalCameraPositionBehind === true) {
    camera.position.set(globalPosBehindX, globalPosBehindY, globalPosBehindZ);
...
renderer.domElement.id = 'bounce-renderer';
...
let flameFirst = new Fire(triggerAnimationTime);
flameFirst.initFire();
// initFire();
...
objCenter = new THREE.Mesh(currentShape, currentMesh);
objCenter.position.set(0, 0, globalPosBehindZ);

// scene.add(obj);
// scene.add(objCenter); //for absolute center reference

scene.add(skyboxCubeMesh); //add nightsky skybox
...
function placeStaticPoolBalls() {
    ...
    for (let i = 0; i <= 13; i++) {
        ...
        // scene.add(poolBalls['ball' + globalLetterNumArr[i]]);
    }
...
let animate = () => {
    ...
    camera.position.x = Math.sin(globalRotation) * 15; //prev: 500
    camera.position.z = Math.cos(globalRotation) * 15;
    ...
    */
    ...
    if (globalAutoScroll === true) {
        ...
        if (globalCameraPositionBehind === true) {
            camera.position.x = globalPosBehindX + (globalTicks);
        } else {
            camera.position.x = (globalTicks) - 12;
        }
    }
    ...
    updateBodies(world);
    world.step(globalFixedTimeStep);
